...
class Can2024App:
    ...
    def show_home(self):
        ...
        # Définir le nombre de colonnes souhaité pour les groupes
        cols_groups = 2

        # Configurer la proportion des lignes et des colonnes pour les groupes
        for i in range(cols_groups):
            self.home_page.columnconfigure(i, weight=1)  # Chaque colonne a le même poids

        total_rows = len(groups_data)
        rows_groups = -(-total_rows // cols_groups)  # Calculer le nombre de lignes nécessaires pour les groupes
        for i in range(rows_groups):
        ...
        for i, (group, teams) in enumerate(groups_data.items()):
            ...
            group_frame.grid(row=i // cols_groups, column=i % cols_groups, padx=10, pady=10, sticky='nsew')

            group_label = tk.Label(group_frame, text=f"Groupe {group}", font=('Helvetica', 14), pady=5)
            group_label.pack()

            for team_info in teams:
                team_frame = tk.Frame(group_frame, bg=None, pady=2, highlightthickness=2, highlightbackground="black")
                team_frame.pack(side='top', fill='both', expand=True)

                team_label = tk.Label(team_frame, text=team_info['Name'], font=('Helvetica', 12), pady=2, cursor='hand2', fg='blue')
                team_label.pack(fill='both', expand=True)
    ...
    def load_groups_data(self):
        # Charger les données depuis le fichier equipes.csv
        groups_data = {}
        with open('./assets/datas/nations/equipes_can2024.csv', newline='', encoding='utf-8') as csvfile:
    ...
    def load_players_data(self, code_name):
        ...
        players_data = []
        file_path = f"assets/datas/teams/{code_name.lower()}.csv"
    ...
    def show_stats(self):
        ...
        equipes_can2024 = pd.read_csv('assets/datas/nations/equipes_can2024.csv')
        ...
        ax.set_title('Comparaison selon le Classement FIFA des Équipes de la CAN 2024 (Par Points)')
